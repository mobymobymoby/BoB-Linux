...
void training_ps(void)
{
    ...
    printf("\"ps\" 명령어의 옵션 표기법은 \"Unix 시스템\"인지 \"BSD 계열 시스템\"인지 \"GNU\"인지에 따라 달라집니다.\n");
    next_line();
    ...
    printf("\"ps\" 명령어에서 같은 옵션 문자여도 대시(dash, '-')가 있는지, 하나인지, 둘인지에 따라 결과가 다릅니다.\n");
    next_line();
    ...
    printf("옵션 없이 \"ps\"만 입력하면 현재 Shell(쉘)이나 터미널에서 실행한 사용자 프로세스에 대한 정보를 출력합니다.\n\n");
    next_line();
    ...
    printf("          (\"ps ax\"만 사용하면 user 정보가 안뜨기 때문에 보통 \"ps aux\"와 같이 사용합니다.\n\n");
    ...
    printf("명령어를 실습해보기 전에 \"ps [옵션]\" 명령의 결과에 나오는 항목들에 대해서 알아보겠습니다.\n\n");
    ...
    printf("이제 \"ps\" 명령어에 Unix 표기 방식과 BSD 표기방식에서 자주 쓰이는 옵션 두개만 실습해보겠습니다.\n\n");
    next_line();
    ...
    printf("\"ps\" 명령어를 입력할 때 \"more\" 명령어를 파이프(pipe, ' | ')로 이어서 입력하면 됩니다\n");
