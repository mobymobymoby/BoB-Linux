...
void training_touch(void)
{
	...
	next_line();
	printf("아무런 옵션을 주지 않고 사용하면 서버의 현재 시간으로 \n파일의 최근 사용 시간(access time), 최근 수정 시간(modify time), 최근 변경 시간(change time)을 변경합니다.\n");
	next_line();
	...
	next_line();
	printf("TIP : 아무런 옵션을 주지 않고 사용하면 서버의 현재 시간으로 \n파일의 최근 사용 시간(access time)과 최근 수정 시간(modify time)을 변경\n");
	printf("$ touch [옵션] [파일명]\n");
	...
	printf("아래는 'ls' 명령어를 통해 출력한 현재 디렉토리에 있는 파일 목록입니다. \ntestfile이라는 빈 파일이 생성된 것을 확인할 수 있습니다.\n");
	printf("\n");
	system("ls");
	next_line();
