...
void training_mkdir(void)
{
	...
	printf("\"mkdir\" 명령어는 make directory의 약자로, '디렉토리'를 생성하는 명령어입니다.\n");
	next_line();
	...
	printf("사용법 : \"mkdir [옵션] [생성할 디렉토리 이름]\"\n");
	...
	printf("[실습] \"mkdir -p\" 명령어를 이용해 'fruits' 디렉토리 안에 'apple' 디렉토리를 생성해보세요.\n");
	run_command("mkdir -p fruits/apple");
	
	printf("[실습] 'ls fruits'를 통해 해당 디렉토리가 제대로 생성되었는지 확인해보세요.\n");
	just_run_command("ls fruits");
	
	printf("mkdir과 대비되는 명령어로는 \"rmdir\"이 있습니다.\n");
