...
void training_mkfs(void)
{
	...
	printf("sda 밑에 sda1, 그리고 sdb가 있는 것을 확인할 수 있습니다. \n우리가 포맷할 디바이스는 'sdb'입니다.\n");
	next_line();
	printf("mkfs는 매우 조심해서 사용해야 합니다. \n잘못 사용하면 정상적인 디바이스를 포맷해버릴 수 있기 때문입니다.\n");
	next_line();
	printf("이것이 바로 mkfs 명령어에 관리자 권한이 필요한 이유입니다.\n");
	next_line();
	printf("mkfs를 이용해서 '/dev/sdb' 파티션을 ext4로 포맷해보세요. \n(단 -v 옵션을 사용하지 않고 실행하세요.)\n");
	...
	next_line();
	printf("[Creating filesystem with 2621440 4k blocks and 655360 inodes]\n해당 파일시스템이 2621440개의 4K 블록과 655360개의 inodes로 생성되었음을 말합니다.\n");
	next_line();
	printf("[Filesystem UUID]\n특정 파일 시스템을 식별하기 위한 값입니다.\n(UUID : Universally Unique IDentifier)\n");
	printf("16bytes로 규격화된 슷자로, 생성시 시간 값을 반영해 겹치지 않도록 고유한 식별 ID를 생성하는 것입니다.\n");
	next_line();
	printf("[Superblock backups stored on blocks] \n슈퍼블록은 리눅스 파일시스템의 중요한 정보가 들어 있는 메타데이터입니다.\n");
	printf("슈퍼블록의 데이터가 훼손되는 것을 막기 위해 리눅스 파일 시스템에서는 슈퍼블록의 복사본을 여러 곳에 저장합니다.\n");
	printf("\n위에서 출력되는 값인 '32768, 98304, 163840, 229376, 294912, 819200, 884736, 1605632'는 복사된 슈퍼블록의 위치입니다.(블록 단위)\n");
	next_line();
