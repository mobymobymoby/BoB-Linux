...
void training_sed(void)
{
	...
	printf("또한 sed 명령어에서 '$'라는 문자는 마지막 줄을 뜻합니다. \n'$'라는 문자는 정규 표현식의 메타 문자중 하나이며, 문자열의 끝을 의미합니다.\n");
	next_line();
	printf("따라서 만약 3번째줄 부터 마지막 줄 까지 출력하고 싶다면 '3,$p'라고 입력하면 됩니다.\n");
	next_line();
	printf("[실습] sed와 -n 옵션을 이용해 textfile 파일의 3~마지막 줄을 출력(p)해 보세요.\n");
	printf("$ sed [옵션] '[출력할 줄 번호]p' [파일의 이름]\n");
	...
	next_line();
	printf("hi라는 단어가 포함된 줄만 출력된 것을 확인할 수 있습니다. \n2번째 줄에는 'This'라는 문자열에 'hi'가 포함되어 출력되었습니다.\n");
	next_line();
	printf("방금전에 보았던 메타 문자인 '$'처럼 '^' 또한 메타 문자로, 문자열의 처음을 나타냅니다.\n");
	next_line();
	printf("^ 문자를 이용하여 특정 문자열로 시작하는 줄을 출력할 수 있습니다.\n");
	next_line();
	printf("2번째 줄은 '2'라는 문자로 시작하기 때문에 \nsed -n '/^2/p' textfile을 통해 2번째 라인을 출력할 수 있습니다.\n");
	next_line();
	printf("[실습] sed와 -n 옵션을 이용해 textfile 파일의 '1'으로 시작하는 줄을 출력(p)해 보세요.\n");
	...
	next_line();
	printf("지금은 메타 문자와 sed의 혼합 사용법이 복잡하지만, \n트레이닝을 반복하여 익숙해질 수 있을 것입니다.\n");
	next_line();

	printf("지금까지는 p를 통한 출력에 대해서 알아보았지만, \n'd'를 이용해서 특정 줄을 제외할 수도 있습니다.\n");
	next_line();
	...
	next_line();
	printf("2번째 줄의 내용이 사라졌습니다. \n리다이렉션은 출력 결과를 파일에 반영할 수 있는 기능으로 유용하게 사용할 수 있습니다.\n");
	next_line();
