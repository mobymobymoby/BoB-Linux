...
void training_mv(void)
{
	...
	printf("사용법 : \"mv [옵션] [원본(디렉토리)] [대상(디렉토리)]\"\n");
	printf("ex) mv /home/rookie/file /home/test/file\n");
	next_line();
	
	printf("옵션은 선택 사항이며 해당 명령어에 지정할 옵션, \n");
	printf("원본은 옮기고자 하는 파일명 또는 디렉토리명,\n");
	printf("대상에는 옮길 위치 또는 바꿀 파일명을 입력합니다.\n");
	next_line();
	
	printf("\"mv\"에서 자주 사용하는 옵션은 다음과 같습니다.\n\n");
	printf("  [ -i (--interactive) ] : 이동할 디렉토리에 동일한 파일명이 존재할 경우 덮어 쓸 것인지 물어봅니다.\n\n");
	printf("  [ -f (--force) ]   : 같은 파일의 이름이 존재하는 경우에 강제로 덮어씁니다.\n\n");
	printf("  [ -b ]             : 덮어쓸 때 백업본 파일을 만듭니다.\n\n");
	printf("  [ -v (--verbose) ] : 파일을 이동할 때 이동 내용을 출력합니다.\n\n");
	printf("  [ -u (--update) ]  : 이동할 디렉터리에 동일한 파일명이 있을 경우 검사하여 해당 파일이 최신 파일이면 이동하지 않습니다.\n\n");
	next_line();
	...
	printf("\n\n\"mv\"는 파일이나 디렉토리를 이동할 때도 사용되지만, 이름을 변경하고자 할 때도 사용됩니다.\n");
