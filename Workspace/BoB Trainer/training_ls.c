...
void training_ls(void)
{
    ...
    printf("'root' 디렉토리(폴더) 밑에 'usr' 디렉토리(폴더) 밑에 'bin' 디렉토리(폴더) 밑에 \"ls\"(으)로 있습니다.\n");
    next_line();
    ...
    next_line();
    printf("'-l' 옵션을 주면 위에 보이는 결과와 같이 리스트 구조로 \n");
    printf("현재 작업 디렉터리에 있는 파일과 디렉토리를 볼 수 있습니다.\n\n");
    next_line();
    printf("만약 명령어 \"ls -l\"을(를) 통해 나온 출력 결과가 다음과 같을 때\n\n");
    printf("\"drwxrw-r-- 1 kim rookie 1024 2020-10-10 test\"\n\n");
    printf("첫 번째 'drwxrw-r--'(은)는 어떤 유형인지와 권한을 나타내고\n");
    ...
    system("clear");
    printf("[\"rwx\"]\n\n");
    printf("그럼 다시 위의 예시로 돌아와서, 먼저 첫 번째 'rwx' 부분은 '사용자(소유자,owner)' 권한입니다.\n");
    ...
    printf("쓰기 권한은 숫자 '2'\n");
    printf("실행 권한은 숫자 '1'\n\n");
    printf("위와 같이 각각의 권한을 의미하는 숫자로 취급하고, 세 권한을 더한 합을 각 자리에 표시합니다.\n\n");
    next_line();
    ...
    next_line();
    //6
    printf("다음으로 '6'은 \"그룹 권한\" 부분이며, 4+2+0이므로, '읽기 권한'과 '쓰기 권한'은 있지만\n");
    ...
    printf("이렇게 \"ls\" 명령어가 무엇을 하고, 어떤 옵션이 있는지에 대해서 배웠고, 실습도 해보았습니다.\n");
