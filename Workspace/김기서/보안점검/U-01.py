...
def U01():
    ...
    if isSafeLogin and isSafeSecuretty and isSafeSshd:
        ...
        report.write('[U-01] 조치 방법\n')
        report.write('\troot 계정의 원격 접속을 차단하세요.\n')
        report.write('\t\t텍스트 에디터를 이용하여 \"/etc/pam.d/login\" 파일을 엽니다.\n')
        report.write('\t\t아래의 내용을 수정 또는 추가하세요.\n')
        report.write('\t\tauth required pam_securetty.so\n')
        report.write('\t\t텍스트 에디터를 이용하여 \"/etc/securetty\" 파일을 엽니다.\n')
        report.write('\t\tpts 관련 내용을 제거하세요.\n')
        report.write('\tssh를 사용 중이라면 ssh를 통한 root 접속을 차단하세요.\n')
        report.write('\t\t텍스트 에디터를 이용하여 \"/etc/ssh/sshd_config\" 파일을 엽니다.\n')
        report.write('\t\tPermitRootLogin 내용을 제거하세요.\n')
