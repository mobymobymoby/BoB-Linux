...
void training_group(void)
{
    ...
    printf("리눅스에서는 사용자(Owner), 그룹(Group), 기타 사용자(Other)로 구분하여 권한을 관리합니다.\n");
    next_line();
    ...
    printf("\"groupadd\" 명령어는 새로운 그룹을 생성하는 명령어기 때문에\n");
    ...
    printf("사용법 : groupdel [삭제할 그룹 이름]\n");
    fake_run_command("groupdel tester");
    next_line();
    ...
    printf("여러 계정을 관리하는 관리자의 입장에서 유연한 계정 관리를 위해 그룹을 활용할 수 있습니다.\n");
	next_line();

    printf("참고로 그룹 관리자는 해당 그룹에 속하지 않아도 됩니다.\n");
    printf("그룹 관리자는 사용자들을 해당 그룹의 2차 그룹으로 묶을 수 있고, 패스워드를 설정할 수 있습니다.\n");
    next_line();
