...
void training_ifconfig(void)
{
    ...
    printf("[flags]는 인터페이스의 상태를 말합니다.\n");
    next_line();

    printf("[mtu]는 네트워크 인터페이스의 최대 전송 단위입니다.\n");
    next_line();
    
    printf("[inet]은 네트워크 인터페이스에 할당된 아이피 주소(ipv4)를 의미하며\n");
    printf("[inet6](은)는 네트워크 인터페이스에 할당된 ipv6 주소를 의미합니다.\n\n");
    printf("일반적으로 '내부 아이피'라 부르는 주소는 ipv4 형식인 \"inet\" 주소입니다.\n");
    next_line();

    printf("[netmask]는 쉽게 생각하여 네트워크에 할당할 수 있는 아이피 주소의 최대 값을 의미합니다.\n");
    next_line();

    printf("[brodcast]는 네트워크에 속하는 모든 호스트에게 데이터를 보낼 수 있는 주소를 의미합니다.\n");
    next_line();

    printf("[ether]는 네트워크 인터페이스의 하드웨어 주소이며, 다른 말로는 맥(MAC) 주소라고 부릅니다.\n");
    next_line();

    printf("\"ifconfig\" 명령어를 사용하여 특정 인터페이스를 비활성화 또는 활성화 할 수도 있습니다.\n");
    printf("사용법 : ifconfig [인터페이스 이름] up/down 으로 사용합니다.\n\n");
    next_line();
    ...
    printf("\n\n위의 실습했던 것처럼 \"ifconfig\" 명령어로 인터페이스를 down 또는 up 할 수 있습니다.\n\n");
    next_line();
