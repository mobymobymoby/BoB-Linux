...
void training_whoami(void)
{
	...
	printf("이번에 배울 명령어는 \"whoami\" 입니다.\n\n");

    printf("이 명령어는 현재 내가 어떤 계정으로 로그인이 되어있는지 궁금할 때 이용하는 명령어입니다.\n");
    next_line();
    
    printf("\"whoami\" 명령어는 현재 로그인한 사용자 계정의 이름, 'EUID(Effective User ID)'를 보여줍니다.\n");
	next_line();
    ...
    printf("'root' 디렉터리(폴더) 밑에 'usr' 디렉터리(폴더) 밑에 'bin' 디렉터리(폴더) 밑에\n");
    printf("\"whoami\"로 있습니다.\n");
	next_line();
    ...
    printf("다른 운영체제에서도 \"whoami\" 명령어를 사용해보시고 다른 명령어들도 한 번씩 실습하여 결과를 확인해보세요.\n");
	next_line();
