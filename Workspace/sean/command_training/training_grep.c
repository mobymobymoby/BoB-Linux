...
void training_grep(void)
{
    ...
    printf("단순히 '문자열을 매칭'하는 것이 아니라 여러 패턴을 찾을 때 사용되는 '정규표현식'을 활용한 '패턴 매칭 방식'을 사용합니다.\n");
    next_line();
    ...
    printf("정규표현식이란\n");
    printf("위키백과 문서에는 \"특정한 규칙을 가진 문자열의 집합을 표현하는데 사용하는 형식언어이다.\"라고 되어있습니다.\n");
    next_line();
    ...
    printf("ex) \"he+llo\" : 문자 'h' 다음에 'e'가 하나 또는 하나 이상 있고 'llo'가 맨 뒤에 붙는 모든 문자열\n\n");
    ...
    printf("메타 문자 '[]'는 '['와 ']' 사이에 '문자 클래스'라고 하여 정규 표현식과는 다르게 가리키는 '문자의 범위'를 의미합니다.\n");
    printf("ex) [a-z]1 : '1' 앞에 소문자 'a'부터 소문자 'z'까지 포함된 모든 문자열\n\n");
    printf("메타 문자 [] 안에 '^' 기호는 위에서의 '지정된 문자열로 시작하는 문자열'로 해석되지 않고 다른 의미로 해석됩니다.\n");
    ...
    printf("'s'로 시작하고 모든 임의의 문자가 반복되고 'g'로 끝나는 문자열을 검색하라는 의미입니다.\n");
    next_line();
