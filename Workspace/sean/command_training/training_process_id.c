...
void training_process_id(void)
{
	...
	printf("프로세스란\n");
	printf("디스크에 저장된 프로그램이 주기억장치인 RAM에 올려져서 운영 체제에 제어되는 상태입니다.\n");
	next_line();

	printf("프로세스화란\n");
	printf("디스크에 저장된 프로그램을 주기억장치인 RAM에 올려서 프로그램이 실행된 상태로 만드는 것 입니다.\n");
	next_line();
	...
	printf("[ PGID ] : 프로세스 그룹을 식별하기 위해 부여뒤는 번호입니다.\n");
	...
	printf("사용자가 로그인할 때 사용한 계정을 의미합니다.\n");
	next_line();
	...
	printf("사용자 계정의 UID가 아니라 실행 파일 소유자의 UID가 됩니다.\n");
	printf("이 때 실행 파일 소유자의 UID를 \"EUID\"라고 합니다.\n");
	next_line();

	printf("2. su 명령을 이용하여 다른 계정으로 전환한 경으.\n\n");
	printf("일반적으로 사용자가 로그인하면 처음에는 UID와 EUID가 같은 값을 갖습니다.\n");
	printf("하지만 \"su\"를 이용하여 다른 사용자로 전환하면 UID와 EUID가 달라지게 됩니다.\n");
	next_line();

	printf("시스템에서 자신이 어떤 계정으로 로그인 되어 있는지 간단하게 알고 싶을 경우 \"whoami\" 명령어를 이용합니다.\n");
	...
	printf("즉, \"SetUID\"는 파일의 권한 부분과 관련하여 일시적으로 UID를 변경하는 것 입니다.\n");
	next_line();

	printf("\"SetUID가 설정된 파일을 실행하면 특정 작업을 수행하기 위해 일시적으로 파일 소유자의 권한을 얻게 됩니다.\n");
	next_line();

	printf("예를 들어 소유자가 'root'인 파일에 SetUID가 설정되어 있다면 일반 사용자가 해당 파일을 실행 했을 때 잠시 동안은 파일의 소유자인 'root'의 권한을 얻게 됩니다.\n");
	next_line();
	...
	printf("그러므로 일반 사용자가 \"passwd\" 파일을 '읽기'는 가능해도 '쓰기'가 불가능하여 수정이 불가능합니다.\n");
	next_line();

	printf("하지만 \"SetUID\"를 이용한다면 일반 사용자여도 \"passwd\" 파일을 수정할 수 있습니다.\n");
	next_line();

	printf("\"SetUID bit\"가 설정된 파일은 일반 사용자가 실행했을 때 일반 사용자의 권한으로 실행하는 것이 아니라\n");
	...
	printf("그러므로 위의 \"/usr/bin/passwd\" 파일은 일반 사용자가 실행했을 때 'root'의 권한으로 실행하게 됩니다.\n");
	next_line();

	printf("이처럼 어떤 사용자라도 \"SetUID\"가 설정된 파일을 실행한다면 파일 소유자의 권한을 잠깐 동안 얻어서 실행합니다.\n");
	next_line();
