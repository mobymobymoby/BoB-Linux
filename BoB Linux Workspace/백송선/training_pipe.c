...
{   
    ...
    next_line();
    printf("[세미콜론(;)]\n");
    printf("\"touch\" 명령어로 test1이라는 파일을 만들고\n\"cat\" 명령어로 test1에 입력을 한 뒤\n\"cat\"명령어로 test1 파일의 내용을 보고 싶다면\n");
    next_line();
    printf("\"touch test1\"\n\"cat > test1\"\n\"cat test1\"\n\n이렇게 3번 입력하여야 합니다.\n");
    next_line();
    printf("하지만 세미콜론(;)을 이용하면\n");
    next_line();
    printf("\"touch test1; cat > test1; cat test1\" 라고 입력하여 한 줄로 3가지 명령어를 실행하게 됩니다.\n");
    next_line();
    printf("게다가 세미콜론(;)은 입력된 명령어들 중 한 개가 실패해도 그 다음 명령어는 실행됩니다.\n");
    printf("즉, 위에 명령어 3개로 예시를 들자면 \"touch test1 \" 명령어가 실패해도 \"cat > test1\"와 \"cat test1\"은 실행됩니다.\n");
    next_line();
    ...
    printf("즉, 앞에서 밀가루 반죽을 뭉쳐서 모양틀을 이용해 모양을 떠서 나온 결과인 별모양 밀가루 반죽을\n");
    printf("->  뒤에서 진행할 오븐에 밀가루 반죽을 넣어 빵 굽는 작업에 별모양 밀가루 반죽을 넣는다 라고 이해하시면됩니다.\n");
    next_line();
    ...
    next_line();
    printf("더블 앰퍼샌드(&&)는 앞에 명령어가 제대로 실행되지 않으면 뒤에 명령어도 실행되지 않기에\n");
    ...
    next_line();
    printf("이렇게 한 줄에서 여러 명령어들을 한 번에 수행할 수 있게 도와주는 기능 4가지를 배웠습니다.\n");
    next_line();
