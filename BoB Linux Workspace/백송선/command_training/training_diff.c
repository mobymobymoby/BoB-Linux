...
void training_diff(void)
{
    ...
    next_line();
    printf("\"diff\" 명령어는 difference의 약자이며 파일을 비교할 때 사용하는 명령어입니다.\n");
    next_line();
    printf("\"diff\" 명령어는 \"cmp\" 명령어보다 더 직관적이고 명확하게 결과를 알려주는 명령어입니다.\n");
    next_line();
    printf("\"diff\" 명령어의 실행 파일은 \"/usr/bin/diff/\"로 있습니다.\n");
    printf("'root' 디렉토리(폴더) 밑에 'usr' 디렉토리(폴더) 밑에 'bin' 디렉토리(폴더) 밑에 \"diff\"로 있습니다.\n");
    next_line();
    ...
    next_line();
    printf("\"diff\" 명령어를 옵션을 넣지 않고 이용하여 'test1 파일'과 'test2 파일'의 내용을 비교해보세요.\n");
    ...
    printf("첫번째 파일에서는 \"you\"라고 적힌 부분이고, 두 번째 파일에서는 \"me\"와 \"okay?\"라고 적힌 부분이다.\n");
    next_line();
    ...
    printf("                              디렉토리 밑에 있는 파일과 디렉토리들도 비교하여 차이점을 출력합니다.\n");
    printf("-y (--side-by-side)         : 두 파일의 차이점을 출력할 때 두 열로 나눠서 출력해줍니다.\n");
    next_line();
    printf("\"diff\" 명령어의 옵션 두 가지만 실습해보겠습니다.\n");
    next_line();
    printf("먼저 '-q' 옵션을 실습해보겠습니다.\n");
    next_line();
    printf("앞에서 만들었던 'test1 파일'과 'test2 파일'을 '-q (--brief)' 옵션을 이용하여 비교해보세요.\n");
    ...
    next_line();
    printf("\"diff\" 명령어에서 '-q (--brief)' 옵션은 두 파일을 비교할 때 차이점의 존재 여부만 출력해주는 옵션입니다.\n");
    next_line();
    printf("다음으로 '-y (--side-by-side)' 옵션을 실습해보겠습니다.\n");
    next_line();
    printf("'test1 파일'과 'test2 파일'을 '-y (--side-by-side)' 옵션을 이용하여 비교해보세요.\n");
    printf("사용법 : diff [옵션] [비교할 파일1] [비교할 파일2]\n");
    ...
    printf("i\t\t\t\t\t\t\t\ti\nlove\t\t\t\t\t\t\t\tlove\nyou\t\t\t\t\t\t\t| me\n\t\t\t\t\t\t\t> okay?\n");
    next_line();
    printf("\"diff\" 명령어에서 '-y (--side-by-side)' 옵션은 두 파일의 차이점을 출력할 때 두 열로 나눠서 출력해주는 옵션입니다.\n");
    next_line();
    ...
    next_line();
    printf("그런데 \"diff\" 명령어 말고 \"diff3\"라는 명령어가 있습니다.\n");
    printf("3개의 파일을 비교할 때 사용하는 명령어입니다.\n");
    next_line();
    ...
    next_line();
    printf("\"ls\"로 test3 파일이 잘 만들어졌는지 확인해봅니다.\n");
    just_run_command("ls");
    next_line();
    printf("이제 \"diff3\" 명령어를 실습해보겠습니다.\n");
    next_line();
    printf("\"diff3\" 명령어를 옵션 없이 이용하여 test1, test2, test3를 비교해보세요.\n");
    ...
    printf("\n해석하자면 이렇습니다.\n");
    next_line();
    printf("먼저, 출력을 보면 '===='이 있는데 이 의미는 3개의 파일 모두 다 다른 점이 있다는 것입니다.\n");
    next_line();
    printf("그리고 '1:', '2:', '3:' 이렇게 되어 있는 것들은 각 파일의 번호 입니다. \n");
    printf("차례대로 1번째 2번째 3번째 파일이라는 것입니다.\n");
    next_line();
    printf("마지막으로 '3c', '3,4c', '3,4c' 이렇게 되어 있는 것들은 각 파일에서 다른 부분의 행 번호 입니다.\n");
    printf("그 밑에는 각 파일에만 입력되어 있고 다른 파일에는 입력이 안된 내용을 출력해 준 것입니다.\n");
    next_line();
    printf("최종적으로 보자면\n");
    printf("1 번째 파일의 3 번째 행 \"you\"라는 내용이 다른 파일에는 없다는 것이고\n");
    printf("2 번째 파일의 3 번째 행과 4번째 행 \"me\"와 \"okay?\"라는 내용이 다른 파일에는 없다는 것이고\n");
    printf("3 번째 파일의 3 번째 행과 4 번째 행 \"it\"과 \"got it?\"이라는 내용이 다른 파일에는 없다는 것입니다.\n");
    next_line();
    printf("만약 1 번째 파일과 2 번째 파일의 내용은 같고 3 번째 내용만 다르다면\n");
    printf("\"====3\"라고 '====' 뒤에 내용이 다른 파일의 번호를 붙여서 출력해줍니다.\n");
    next_line();
    printf("\"diff3\" 명령어도 옵션이 존재합니다만, 이 트레이닝에서 \"diff3\" 옵션에 대해서는 다루지 않겠습니다.\n");
    next_line();
