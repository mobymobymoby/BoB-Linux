...
void training_cmp(void)
{
    ...
    printf("파일 비교 명령어는 은근히 유용하게 쓸 수 있는 명령어입니다.\n");
    next_line();
    ...
    printf("기본값으로 만일 두 파일의 내용이 같다면 아무런 출력도 되지 않습니다.\n");
    next_line();
    printf("\"cmp\" 명령어는 두 개의 파일을 비교하고 나서 다른지 여부만 확인하기 때문에\n");
    printf("아무 옵션 없이 사용될 경우, 첫번 째 다른 문자가 발견되면 다른 문자가 발견된 위치만 출력하고 끝나버립니다.\n");
    printf("즉, 두 파일을 비교할 때 다른 문자가 나오면 그 부분만 출력하고 뒤에 다른 문자들은 출력되지 않고 끝나버립니다.\n");
    next_line();
    printf("\"cmp\" 명령어는 두 개의 파일의 내용이 서로 같은지 혹은 다른지에 대한 참과 거짓 형태를 얻을 때 유용합니다.\n");
    next_line();
    printf("예를 들어 두 개의 파일이 있는데 다른 부분이 있는지 알고 싶을 때\n");
    printf("다른 부분이 있는지 없는지에 대한 여부를 바로 알 수 있습니다.\n");
    next_line();
    printf("하지만 \"cmp\" 명령어의 단점은 서로 다른 부분이 있다면 존재 여부만 확인 할 수 있지\n");
    ...
    just_run_command("ls");
    next_line();
    printf("파일 두 개의 내용이 다른지 같은지 참 혹은 거짓 형태로 출력되게 실습해보겠습니다.\n");
    next_line();
    printf("\"cmp\" 명령어를 이용하여 옵션을 넣지 않고 test1 파일과 test2 파일의 내용을 비교해보세요.\n");
    printf("사용법 : cmp [비교할 파일1] [비교할 파일2]\n");
    just_run_command("cmp test1 test2");
    printf("\n학습을 잘 따라오셨다면, \"test1 test2 differ : byte 26, line 4\"라고 뜰 것입니다.\n");
    next_line();
    printf("해석하자면 이렇습니다.\n");
    printf("\"파일 test1\"과 \"파일 test2\"의 다른 부분은 '4번째 행'이고\n");
    printf("'공백과 엔터 포함하여 전체 문자 중 26번째 문자'이다.\n");
    next_line();
    printf("\"cmp\" 명령어의 자주 쓰이는 옵션은 다음과 같습니다.\n");
    next_line();
    printf("-b (--print-bytes) : 두 개의 파일을 비교 시 두 파일 간의 최초로 다른 문자들을 ASCII 값 8진수로 출력합니다.\n");
    printf("-i (--ignore-initial) : 지정된 바이트 수 만큼 무시하고 다음 문자부터 1로 시작하여 비교합니다.\n");
    printf("-l (--verbose) : 두 파일 간의 다른 문자에 대한 위치와 그 문자들의 8진수 값을 출력합니다.\n");
    printf("-s (--silent) : 사용자에게 아무것도 출력하지 않습니다. 하지만 시스템상에서는 실행 결과 코드 값이 반환합니다.\n");
    printf("                (0 - 파일의 내용이 같다. / 1 - 파일의 내용이 다르다. / 2 - 파일에 접근할 수 없다)\n");
    next_line();
    printf("\"cmp\" 명령어의 옵션 두 가지만 실습해보겠습니다.\n");
    next_line();
    printf("먼저 '-i (--ignore-initial)' 옵션을 실습해보겠습니다.\n");
    next_line();
    printf("미리 만들었던 test1 파일과 test2 파일을 '-i 6' 옵션을 이용하여 비교해보세요.\n");
    printf("사용법 : cmp [옵션] [비교할 파일1] [비교할 파일2]\n");
    just_run_command("cmp -i 6 test1 test2");
    next_line();
    printf("\"test1 test2 differ: bytes 20, line 3\"라고 출력이 되었을 것입니다.\n");
    next_line();
    printf("출력값이 다르다는 걸 느끼셨나요?\n");
    next_line();
    printf("파일의 내용은 하나도 건들이지 않았는데, 처음엔 \"test1 test2 differ: bytes 26, line 4\"라고 출력되었던 값이 달라졌습니다.\n");
    next_line();
    printf("눈치 못 채셨어도 괜찮습니다. 이유를 알려드릴게요\n");
    next_line();
    ...
    next_line();
    printf("미리 만들었던 test1과 test2를 '-s' 옵션을 이용하여 비교해보세요\n");
    printf("사용법 : cmp [옵션] [비교할 파일1] [비교할 파일2]\n");
    just_run_command("cmp -s test1 test2");
    next_line();
    printf("화면상에 아무런 결과값이 출력되지 않았습니다.\n");
    next_line();
    printf("하지만!\n");
    next_line();
    printf("명령어는 정상 동작 하였습니다.\n");
    next_line();
    printf("'-s' 옵션은 사용자에게 출력해주지 않지만, 출력만 안될 뿐 시스템에서는 서로 같을 때와 다른 때에 대한 코드값을 반환했습니다.\n");
    printf("코드 값 : 0 - 파일의 내용이 같다. / 1 - 파일의 내용이 다르다. / 2 - 파일에 접근할 수 없다.\n");
    next_line();
    printf("\"cmp\" 명령어는 아주 간단하게 파일을 비교할 때 쓰이는 명령어입니다.\n");
    printf("뒤에 또 다른 파일 비교 명령어들을 배울 것입니다.\n");
    next_line();
    printf("\"cmp\" 명령어의 옵션은 위에 나온 것들 말고도 여러가지가 있습니다.\n");
    next_line();
