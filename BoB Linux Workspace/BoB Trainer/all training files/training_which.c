...
void training_which(void)
{
    ...
    next_line();
    printf("\"which\" 명령어의 실행 파일은 \"/usr/bin/which\"에 위치해 있습니다.\n");
    printf("이를 풀어서 설명하자면, 'root' 디렉토리(폴더) 밑에 'usr' 디렉토리(폴더) 밑에\n");
    printf("'bin' 디렉토리(폴더) 밑에 \"which\"로 있습니다.\n");
    next_line();
    printf("사용법은 \"which [옵션] [위치를 찾고 싶은 명령어]\" 입니다.\n");
    printf("ex)which ls\n");
    next_line();
    printf("\"which\" 명령어는 명령어의 위치를 알려주는 간단한 명령어입니다.\n");
    next_line();
