...
void training_chsh(void)
{
    ...
    printf("콘솔이나 터미널에서 사용자가 문자를 입력하여 명령을 내리면 그 명령을 해석하고 실행해줍니다.\n");
    next_line();
    
    printf("일종의 사용자가 내리는 명령을 시스템이 수행할 수 있도록 중간에서 브로커 같은 역할을 하는 비서입니다.\n");
    next_line();
    ...
    printf("초기의 유닉스 쉘인 \"sh\"와 대부분 호환되고, 현재 리눅스 배포판 대부분의 기본적인 쉘입니다.\n");
    next_line();
    ...
    printf("\"chsh\" 명령어의 옵션은 '-s (--shell)'와'-R (--root)'이 있지만 '-s'만 알아둬도 됩니다.\n");
    next_line();
