...
void training_mv(void)
{
	...
	next_line();
	printf("-i, --interactive : 이동할 디렉토리에 동일한 파일명이 존재할 경우 덮어 쓸 것인지 물어봅니다. (yes - y /no - n)\n-f, --force : 같은 파일의 이름이 존재하는 경우에 강제로 덮어씁니다.\n-b : 덮어쓸 때 백업본 파일을 만듭니다.\n-v, --verbose: 파일을 이동할 때 이동 내용을 출력합니다.\n-u, --update : 이동할 디렉터리에 동일한 파일명이 있을 경우 검사하여 해당 파일이 최신 파일이면 이동하지 않습니다.\n");
	next_line();
