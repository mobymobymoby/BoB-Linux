...
void training_sudo(void)
{
	...
	next_line();
	printf("여러분들이 경험을 쌓아 하나의 서버를 관리하게 된다면,\nsudoers 파일을 신중하게 작성하게 될 것입니다.\n");
	next_line();
	printf("특히, 위 처럼 모든 명령어에 대한 권한을 사용자에게 주는 경우\n해당 사용자의 권한이 너무 커져버리기 때문에 적절한 조치는 아닙니다.\n");
	next_line();
	printf("일반 사용자에게는 필요한 만큼의 최소 권한만 주는 것이 \n보안을 위한 권한 관리의 기본이라고 할 수 있겠습니다.\n");
	       
	printf("참고로 다른 사용자로 변경하는 명령어로는 'su' 명령어가 있습니다.\n'sudo'와 글자가 비슷하므로, 혼동하지 않도록 합니다.\n");
	next_line();
