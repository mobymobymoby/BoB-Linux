...
void training_sed(void)
{
	...
	next_line();
	printf("[실습] sed와 -n, -e 옵션을 이용해 textfile 파일의 4번째 줄과 7~9번째 줄을 동시에 출력(p)해 보세요.\n");
	...
	next_line();
	printf("[실습] sed와 -n 옵션을 이용해 textfile 파일의 'hi'라는 단어가 포함된 줄을 출력(p)해 보세요.\n");
	...
	next_line();
	printf("[실습] 이번에는 textfile의 2번째 줄을 제외한 내용을 실제로 'new_textfile'에 저장해보세요.\n사용법은 다음과 같습니다.\n");
	...
	next_line();
	printf("[실습] s 옵션을 이용하여 textfile의 'l'라는 문자를 'here'로 바꿔 new_textfile에 저장해보세요. 단 일치하는 모든 문자가 바뀌도록 해보세요.\n");
