...
void training_wget(void)
{
    ...
    next_line();
    printf("wget을 통해 다운로드 받은 파일에 대한 정보가 출력됩니다.\n\n");
    printf("접속한 URL의 IP 정보와 '200'이라는 HTTP 상태코드 등\n여러가지 정보를 확인할 수 있습니다.\n");
    printf("또한, 우리가 방금 다운로드 받은 파일은 용량이 낮은 파일이라 확인하기 힘들었겠지만,\n파일이 다운로드 받아지는 상태가 표시됩니다.\n");
	
    printf("\n다운로드 받은 파일인 'index.html'을 확인해보겠습니다.\n");
    printf("[실습] cat을 이용하여 index.html의 내용을 확인해보세요.\n");
    just_run_command("cat index.html");
	
    printf("우리가 네이버에 접속하여 볼 수 있는 내용이 텍스트로 출력됩니다.\n\n");
    printf("\n이렇게 html 파일을 다운로드 받을 수도 있지만,\n우리가 흔히 인터넷에서 다운로드 받는 자료들도 wget으로 받을 수 있습니다.\n");
    printf("인터넷에서 클릭을 통해 다운로드를 받는 방식은\n어떤 아이콘을 클릭하면 그에 해당하는 링크로 이동되는 원리입니다.\n\n");
    printf("따라서 해당 링크 주소를 얻을 수 있다면, \n동일한 파일을 wget [해당 주소] 명령어를 통해 다운로드 받을 수 있습니다.\n");
