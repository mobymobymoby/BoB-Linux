...
void training_su(void)
{
	...
	printf("[실습] 가상으로 su 명령어를 사용해 'test' 사용자로 전환해보세요. (su [전환할 사용자 이름])\n");
	fake_run_command("su test");
	
	printf("Password:\n");
	printf("su 명령어로 계정을 전환할 때는 바꿀 계정의 비밀번호를 입력해야 합니다.\n");
	next_line();
	...
	next_line();
	printf("지금은 '환경 변수'라는 개념에 대해 잘 모를 수 있지만,\n리눅스의 경험이 쌓이고 나면 어떤 의미인지 이해할 수 있을 것입니다.\n");
	next_line();
	printf("환경 변수의 개념을 다루고 있는 명령어로는 env, export 등이 있습니다.\n해당 명령어를 학습하여 환경 변수의 개념을 익혀보세요!\n");
	next_line();
