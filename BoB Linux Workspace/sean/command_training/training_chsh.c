...
void training_chsh(void)
{
    ...
    next_line();
    printf("\"chsh\" 명령어는 Change Shell의 약자로 Shell을 바꿔주는 명령어입니다.\n");
    next_line();
    ...
    next_line();
    printf("이것이 바로 Shell이고 사용자가 명령을 입력하길 기다리고 있는 상태입니다.\n");
    next_line();
    printf("현재 사용자가 어떤 Shell을 사용 중인지 확인하기 위해 \"cat /etc/passwd\"라고 입력해보세요.\n");
    just_run_command("cat /etc/passwd");
    printf("\n위에 출력값 중 맨 왼쪽에 자신의 계정 이름인 %s이 적힌 부분을 찾아 맨 오른쪽을 보시면\n", getlogin());
    printf("\"/bin/bash\" 혹은 자신이 사용 중인 Shell이 적혀 있을 것입니다.\n");
    next_line();
    printf("\"bash\"는 여러 Shell 중에 하나인데\n");
    printf("\"Borne Again Shell\"의 약자이며 처음으로 만들어진 쉘이자 리눅스의 기본적인 쉘입니다.\n");
    next_line();
    ...
    printf("\"ls /bin/*sh\"라고 입력해보세요.\n");
    just_run_command("ls /bin/*sh");
    printf("\n위에 기본으로 설치되어있는 Shell들이 보일 것입니다.\n");
    next_line();
    ...
    printf("지금 바로 Shell이 바뀌지만, 로그아웃을 했다가 로그인하며 원래 Shell로 돌아옵니다.\n");
    next_line();
    printf("반면에 \"chsh\" 명령어로 Shell을 변경하시면\n");
    printf("다음 로그인 때무터 적용되지만, 로그아웃을 했다가 로그인 해도 돌아가지 않습니다.\n");
    next_line();
    ...
    printf("\"cat /etc/passwd\"를 입력하여 자신의 Shell을 확인해보세요.\n");
    just_run_command("cat /etc/passwd");
    printf("\n자신의 계정 옆에 /bin/sh라고 적힌 걸 확인하실 수 있을 것입니다.\n");
    next_line();
    printf("하지만 \"echo $SHELL\"이라고 명령어를 입력해서 확인해보세요.\n");
    just_run_command("echo $SHELL");
    printf("\n아직 Shell이 \"/bin/sh\"로 바뀌지 않은 것을 보실 수 있습니다.\n");
    next_line();
    printf("그 이유는 아까 말했듯이 \"chsh\" 명령어로 바꾸면 한 번 로그아웃 후 다시 로그인을 해야 적용된다는 번거로움이 있지만\n");
