...
void training_ps(void)
{
    ...
    next_line();
    printf("주로 \"ps -ef\"와 \"top\" 이 두 명령어를 통해 CPU 사용률과 사용 중인 프로세스를 확인합니다.\n");
    next_line();
    printf("\"ps\" 명령어의 옵션 표기법에는 \"Unix 시스템\"인지 \"BSD 계열 시스템\"인지 \"GNU\"인지에 따라 달라집니다.\n");
    next_line();
    ...
    printf("-a : 일반적으로 로그인 쉘을 제외하고 데몬 프로세스처럼 터미널에 종속되지 않은 모든 프로세스를 출력합니다.\n");
    ...
    printf("-u : 특정 사용자의 프로세스 정보를 확인할 때 사용합니다. (기본값으로는 현재 사용자로 되어있습니다.)\n");
    ...
    printf("    (\"ps ax\"만 사용하면 user 기준의 정보가 안뜨기 때문에 보통 \"ps aux\"와 같이 사용합니다.\n");
    ...
    next_line();
    printf("\"ps -ef\"를 입력하여 프로세스 정보들을 확인해보세요.\n");
    ...
    next_line();
    printf("\"ps -ef | more\"라고 입력하여 결과를 한 페이지씩 확인해보세요.\n");
    just_run_command("ps -ef | more");
    printf("\n잘 입력하셨다면\"ps -ef\"의 결과를 한 페이지씩 보실 수 있었을 것입니다.\n");
    next_line();
    printf("이번에는 \"ps aux\"를 입력하여 프로세스 정보들을 확인해보세요.\n");
