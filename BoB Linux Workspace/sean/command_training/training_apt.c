...
void training_apt(void)
{
    ...
    next_line();
    printf("\"apt\"는 패키지 관련 관리 도구이기 때문에 대부분 관리자 권한으로 실행되어야 하므로 sudo와 함께 사용합니다.\n");
    next_line();
    ...
    next_line();
    printf("실제 Ubuntu 14 이상부터는 \"apt-get\"이나 \"apt-cache\"대신 \"apt\" 사용하도록 권장하고 있습니다.\n");
    next_line();
    printf("하지만 \"apt-get\" 명령어가 예전부터 오래 사용되오던 명령어이므로 제공되는 기능도 많고 조금 더 안정적이긴 합니다.\n");
    next_line();
    ...
    next_line();
    printf("그런데 위에서 말한 의존성의 문제라는 것은 뭘까요\n");
    next_line();
    printf("쉽게 말하자면\n");
    printf("A라는 프로그램을 실행하려는데 B라는 프로그램이 있어야만 A 프로그램이 실행되는 것입니다.\n");
    next_line();
    printf("에를 들어서 사람이 살아가면서 활동을 하려면 에너지가 필요합니다.\n");
    ...
    printf("무언가를 실행하기 위해서 그 전에 있어야 할 무언가가 필요한데 그 무언가가 없어서 실행할 수 없다면\n");
    next_line();
    printf("이것이 의존성 문제입니다.\n");
    printf("즉, 특정 프로그램을 실행할 때 다른 프로그램에 의존하는 것으로\n");
    printf("의존하는 다른 프로그램이 있어야만 실행되는 것을 의미합니다.\n");
    next_line();
    printf("의존성 문제를 해결해 주는 방법으로는 특정 프로그램이 의존하는 다른 프로그램들을 설치해주는 것입니다.\n");
    next_line();
    printf("그런데 특정 프로그램이 의존하는 다른 프로그램들이 많다면 사용자 입장에서는 엄청 힘들겠지만\n");
    printf("자동 설치 도구는 의존성 문제를 해결해주어 쉽게 패키지 설치가 가능합니다.\n");
    next_line();
    printf("Redhat에 패키지 관리 도구로 \"rpm\"이 있다면, Debian에는 \"dpkg\"가 있고\n");
    printf("Redhat에 자동 설치 도구로 \"yum\"이 있다면, Debian에는 \"apt\"가 있습니다.\n");
    next_line();
    printf("Debian 계열에서 '패키지(실행파일)'를 설치하기 위해 필요한 '설치파일'의 확장자명은 '.deb' 입니다.\n");
    next_line();
    printf("그리고 해당 파일이 뭘 설치하기 위한 것인지에 대한 정보를 담는 일종의 작성 방식 약속이 있습니다.\n");
    next_line();
    printf("그 약속은 \"패키지이름_버전-릴리즈-아키텍쳐.deb\" 라고 표기하는 것입니다.\n");
    printf("ex)vim_8.0.1453-1ubuntu1_amd64.deb\n");
    next_line();
    printf("하지만 \"dpkg\" 역시 의존성 문제를 해결해 주지 못합니다.\n");
    printf("프로그램 설치에 있어서는 의존성 문제도 해결해주는 \"apt\" 명령어가 좋습니다.\n");
    next_line();
    printf("이제 \"apt\" 명령어의 사용법에 대해 알아보겠습니다.\n");
    next_line();
    printf("참고 사항으로 \"apt\" 대신 \"apt-get\" 사용해도 되지만\n");
    printf("\"apt\"가 더 처리할 수 있는 범주가 넓기 때문에 권장되고 있습니다.\n");
    next_line();
    printf("\"apt\" 명령어를 사용할 때는 사용자로부터 동의를 구할 수 있습니다.\n");
    printf("명령어를 입력한 후 'y'혹은 'n'을 입력하라고 나오면 'y'키 혹은 엔터를 눌러 진행해주세요.\n");
    next_line();
    ...
    next_line();
    printf("아래의 내용은 \"apt\" 명령어들입니다.\n");
    next_line();
    printf("sudo apt update                 : 패키지 인덱스 정보(/etc/apt/sources.list)를 업데이트 합니다.\n");
    ...
    printf("sudo apt list --installed       : 설치된 패키지 목록을 조회합니다. (wc -l 명령어를 pipe로 이어주면 숫자로 표기됩니다.)\n");
    printf("sudo apt list --upgradable      : 설치된 패키지 중 업그레이드 대상 패키지 목록을 조회합니다. (wc -l 명령어를 pipe로 이어주면 숫자로 표기됩니다.)\n");
    printf("sudo apt list --all-versions    : 패키지의 모든 버전 목록을 조회합니다. (wc -l 명령어를 pipe로 이어주면 숫자로 표기됩니다.)\n\n");
    next_line();
    printf("\"apt\" 명령어 실습에서는 정말 시스템의 패키지를 대상으로 하게 됩니다.\n");
    next_line();
    printf("현재 진행 중인 실습 명령어를 종료하고 다음 실습 명령어로 넘어가고 싶다면\n");
    ...
    next_line();
    printf("\"sudo apt udpate\"를 입력하여 패키지 인덱스 정보를 업데이트 해보세요.\n");
    just_run_command("sudo apt update");
    printf("\n잘 입력하셨다면 패키지 인덱스 정보가 업데이트 되었을 것입니다.\n");
    next_line();
    printf("다음으로 설치되어 있는 패키지들을 새 버전으로 업그레이드해보겠습니다.\n");
    next_line();
    printf("\"sudo apt upgrade\"를 입력하여 설치되어 있는 패키지들을 새 버전으로 업그레이드 해보세요.\n");
    just_run_command("sudo apt upgrade");
    printf("\n잘 입력하셨다면 설치 되어 있는 패키지들이 새 버전으로 업그레이드 되었을 것입니다.\n");
    next_line();
    printf("이번에는 특정 패키지를 설치해보겠습니다.\n");
    next_line();
    printf("\"sudo apt install gcc\"를 입력하여 gcc 패키지를 설치해보세요.\n");
    just_run_command("sudo apt install gcc");
    printf("\n잘 입력하셨다면 gcc이 정상적으로 설치되었을 것입니다.\n");
    next_line();
    printf("마지막으로 방금 설치한 gcc 패키지를 제거해보겠습니다.\n");
    next_line();
    printf("\"sudo apt remove gcc\"을 입력하여 gcc 패키지를 제거해보세요.\n");
    just_run_command("sudo apt remove gcc");
    printf("\n잘 입력하셨다면 정상적으로 gcc 패키지가 제거되었을 것입니다.\n");
    next_line();
    printf("하지만 'remove' 옵션은 설정 파일까지 제거하지는 않습니다.\n");
    printf("'purge' 옵션을 이용해야 대부분 설정파일까지 제거할 수 있습니다.\n");
    next_line();
