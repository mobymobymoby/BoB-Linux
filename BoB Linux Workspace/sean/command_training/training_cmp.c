...
void training_cmp(void)
{
    ...
    printf("아무 옵션 없이 사용될 경우, 첫번 째 서로 다른 문자가 발견되면\n");
    printf("서로 다른 문자가 발견된 위치만 출력하고 끝나버립니다.\n\n");
    printf("즉, 두 파일을 비교할 때 맨 처음 내용이 다른 문자가 나오면 그 부분만 출력하고 뒤에 다른 문자들은 출력되지 않고 끝나버립니다.\n");
    next_line();
    ...
    next_line();
    printf("미리 만들었던 'test1 파일'과 'test2 파일'을 '-i 6' 옵션을 이용하여 비교해보세요.\n");
    ...
    next_line();
    printf("'-i 6'는 6byte 즉, 공백 및 엔터를 포함하여 6개의 문자를 무시하고 그 다음 것부터 1로 세라는 옵션입니다.\n");
    next_line();
    ...
    next_line();
    printf("미리 만들었던 'test1 파일'과 'test2 파일'을 '-s' 옵션을 이용하여 비교해보세요\n");
    printf("사용법 : cmp [옵션] [비교할 파일1] [비교할 파일2]\n");
    just_run_command("cmp -s test1 test2");
    next_line();
    printf("화면상에 아무런 결과값이 출력되지 않았을 것입니다.\n");
    next_line();
