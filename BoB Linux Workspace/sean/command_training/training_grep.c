...
void training_grep(void)
{
    ...
    next_line();
    printf("\"grep\" 명령어는 입력으로 전달된 파일의 내용에서 특정 문자열을 찾고자 할 때 사용하는 명령어입니다.\n");
    next_line();
    printf("\"grep\" 명령어의 실행 파일은 \"/bin/grep\"으로 있습니다.\n");
    printf("'root' 디렉토리(폴더) 밑에 'bin' 디렉토리(폴더) 밑에 \"grep\"으로 있습니다.\n");
    next_line();
    printf("\"grep\" 명령어는 리눅스에서 자주 사용되는 명령어들 중 하나입니다.\n");
    next_line();
    printf("\"grep\" 명령어는 파일로부터 입력을 받아 문자열을 검색할 때 단순히 문자열을 단순 문자열 매칭이 아니라\n");
    printf("정규 표현식을 활용한 패턴 매칭 방식을 사용하기 때문입니다.\n");
    next_line();
    printf("그리고 \"grep\" 명령어는 디렉토리 내에서 지정된 패턴을 포함하는 파일을 출력할 수도 있습니다.\n");
    next_line();
    ...
    next_line();
    printf("위의 사용법 내용 중 [패턴] 부분에는 찾고자 하는 문자열을 찾을 때 사용 될 정규표현식(정규식)이 들어갑니다.\n");
    next_line();
    printf("문자열에는 공백, 구두점, 제어문자 등이 포함됩니다.\n");
    next_line();
    printf("[파일명] 부분에는 파일명을 따로 지정하면 해당 파일에서만 검색하지만\n");
    printf("애스터리스크(*)와 같은 메타문자를 사용하면 현재 디렉토리의 모든 파일을 대상으로 문자열을 검색합니다.\n");
    next_line();
    printf("\"grep\" 명령어는 주로\n");
    printf("앞의 명령어 실행 결과로 나온 출력을 다음 명령어의 입력으로 이어지게 해주는\n");
    printf("'pipe(|)'와 같이 조합하여 이용됩니다.\n");
    next_line();
    printf("앞에서 배운 \"ps\" 명령어와 조합하여 \"ps aux | grep sh\"를 입력해보세요.\n");
    ...
    next_line();
    printf("정규표현식이란\n");
    printf("위키백과 문서에는 \"특정한 규칙을 가진 문자열의 집합을 표현하는 데 사용하는 형식언어이다.\"라고 되어있습니다.\n");
    next_line();
    ...
    printf("*       : 반복을 나타내는 메타 문자로, 메타 문자 '*' 앞에 적힌 글자가 0번 이상 반복되는 모든 문자열을 검색합니다.\n");
    printf("{n, m}  : 이 또한 반복을 나타내는 메타 문자로, 위의 '*'와는 다르게 반복 횟수를 n번 이상 m번 이하로 지정할 수 있습니다.\n");
    printf("^       : 검색하고자 하는 문자열로 시작하는 문자열의 앞에 써줌으로서\n");
    printf("         \"^hello\"와 같이 하면 \"hello\"라는 문자열로 시작하는 문자열들을 검색합니다.\n");
    printf("$       : 위의 '^'와는 반대로 검색하고자 하는 문자열로 끝나는 문자열의 뒤에 써줌으로서\n");
    printf("         \"world$\"와 같이 하면 \"world\"라는 문자열로 끝나는 문자열들을 검색합니다.\n");
    printf("+       : 메타 문자 '+' 바로 앞의 문자가 하나 이상 있는 모든 문자열을 검색합니다.\n");
    printf("          ex) \"he+llo\" : 문자 'h' 다음에 'e'가 하나 또는 하나 이상 있고 'llo'가 맨 뒤에 붙는 모든 문자열\n");
    printf("?       : 메타 문자 '?' 바로 앞에 문자가 없거나 하나인 모든 문자열을 검색합니다.\n");
    printf("          ex) \"wh?y\" : 'w'와 'y' 사이제 'h' 문자가 하나 있거나 없는 모든 문자열\n");
    printf("|       : '또는', 'or'과 같은 뜻으로 선택문에 쓰입니다.\n");
    printf("          ex) hello|world : 'hello' 또는 'world' 문자나 문자열\n");
    printf("\\       : 메타 문자 앞에 붙여줌으로서, 메타 문자가 가진 의미를 없애고 문자로 취급합니다.\n");
    printf("          ex) \\. : 원래 임의의 문자 한 개를 의미하지만, '\\'가 붙었으므로 문자 '.' 으로 취급\n");
    printf("[ ]     : 메타 문자 '['와 ']' 사이에 있는 문자 한개를 의미합니다.\n");
    printf("          ex) [h]i : 'i' 앞에 'h'가 붙은 문자열\n");
    printf("         '[]'는 엄청 많은 활용 방법이 있습니다.\n");
    printf("          메타 문자 '[]'는 '['와 ']' 사이에 '문자 클래스'라고 하여 정규 표현식과는 다르게 가리키는 문자의 범위를 의미합니다.\n");
    ...
    printf("'{}'    : 메타 문자 '{'와 '}' 사이에 있는 숫자는 '{}' 앞에 붙은 문자의 개수를 의미합니다.\n");
    printf("          ex) a{4}b : 'b' 앞에 'a'가 4개 붙은 문자열\n");
    printf("          또한 '{}' 메타 문자는 숫자의 범위를 지정할 수 있습니다.\n");
    printf("          ex) a{1,4}b : 'b' 앞에 'a'가 1개 이상 4개 이하가 붙은 문자열\n");
    printf("          ex) a{,4}b  : 'b' 앞에 'a'가 없거나 4개 이하가 붙은 문자열\n");
    printf("          ex) a{4,}b  : 'b' 앞에 'a'가 4개 이상 붙은 문자열\n");
    printf("'()'    : '('와 ')' 사이에 들어가는 문자열은 하나로 묶어서 하나의 문자로 다룹니다.\n");
    ...
    printf("-F : \"grep\" 명령어의 기본 사용 문법에서 [패턴] 부분을 정규 표현식이 아닌 일반 문자열로 해석합니다.\n");
    printf("-z : 라인을 줄 바꿈('\\n')이 아닌 NULL(\\0)로 구분합니다.\n");
    next_line();
    ...
    next_line();
    printf("\"grep -i \"world\" test.txt\"를 입력하여 -i 옵션을 실습해보세요.\n");
    just_run_command("grep -i \"world\" test.txt");
    printf("잘 따라하셨다면 \"Hello world\"와 \"good morning world\"가 출력되었을 것입니다\n");
    next_line();
