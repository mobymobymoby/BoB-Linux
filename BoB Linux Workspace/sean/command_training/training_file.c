...
void training_file()
{
    ...
    printf("' -b (--brief)  : 지정한 파일명은 출력하지 않고 파일의 유형만 출력합니다.\n");
    printf("' -f (--file-from [파일 목록]) : [파일 목록]에서 지정한 파일들에 대해서만 명령을 실행합니다.\n");
    printf("' -i (--mime)   : MIME 타입 문자로 출력합니다.\n");
    printf("' -L (--dereference)    : 심볼릭 링크된 파일을 추적하여 원본 파일 정보를 출력합니다.\n");
    printf("' -m (--magic-file [매직 파일] : 임의로 [매직 파일]을 지정합니다.\n");
    printf("' -z (--umcompress)     : 압축된 파일의 내용을 출력해줍니다.\n");
    next_line();
    ...
    printf("\n아까 실습했던 \"file test.txt\"(와)과는 다르게 파일의 이름은 출력해주지 않고\n");
