...
void training_vi(void)
{
    ...
    printf("이번에 배울 것은 \"vi\"와 \"vim\" 입니다.\n");
    next_line();
    printf("\"vi\"와 \"vim\"은 다른 프로그램입니다.\n");
    next_line();
    printf("하지만 요즘 대다수의 유닉스와 리눅스 배포판에서 vi를 입력하면 'vim'로 입력되어 'vim'이 실행됩니다.\n");
    next_line();
    printf("이 때 vim은 vi와 호환이 되는 것으로 본래의 vi가 실행되는 것이 아닙니다.\n");
    next_line();
    printf("사용법은 \"vi [file.txt]\" 입니다.\n");
    printf("ex) vi test1.txt");
    next_line();
    printf("요즘은 수 많은 편집기들이 있고 GUI화가 일반적인 윈도우 같은 경우는 편집과 다른 기능들을 추가한 프로그램들도 여럿 존재합니다.\n");
    next_line();
    printf("리눅스에서도 대표적으로 'Emacs'라는 유료였지만 이제는 무료로 사용할 수 있는 편집기와 'nano'가 있습니다.\n");
    next_line();
    printf("\"vi\"는 '빌 조이'라는 분이 1976년에 만드신 Unix 계열 운영 체제에서 쓰이는 문서 편집기이며\n");
    printf("\"vi\"는 'Visual editor' 라는 뜻입니다.\n");
    next_line();
    printf("\"vi\"가 있기 전에 라인 에디터 \"ed\"라는 편집기가 있었습니다.\n");
    next_line();
    printf("요즘에는 그래픽이 더해져서 사용자들이 문서를 편집하고 텍스트를 편집하기 쉽지만\n");
    next_line();
    printf("옛 날에 사용된 라인 에디터\"ed\"는 잉크로 글자를 찍어서 보여주는 시대에 탄생한 것으로\n");
    printf("'어디부터 어디까지 보여줘라'라고 명령해야 보여주고 편집이 가능했습니다.\n");
    next_line();
    printf("\"ed\"는 POSIX 표준으로 지정되어 있기 때문에 유닉스 호환 시스템에는 설치 되어있거나 설치할 수 있습니다.\n");
    next_line();
    printf("하지만 \"ed\"는 전문가가 아닌 일반 사용자들이 사용하기에 어려움이 있었고 친절하지 못했습니다.\n");
    next_line();
    printf("그래서 BSD 유닉스를 만들던 버클리 대학에서 '빌 조이'라는 프로그래머 분이\n");
    printf("\"visual mode\"라는 것을 코딩하여 플러그인으로 만들어 넣었다고 알려져 있습니다.\n");
    next_line();
    printf("이후에 라인 편집기에 플러그인 형태로 들어가 있던 \"vi\"가 인기가 좋아 분리되어 하나의 프로그램으로 만들어졌습니다.\n");
    next_line();
    printf("그리고 이 당시 Emacs가 유료였기도 하고, BSD에 기본으로 설치되어 있는 편집기가 \"ed\"와 \"vi\"뿐이었기 때문에 인기가 많았습니다.\n");
    next_line();
    printf("하지만\n");
    next_line();
    ...
    printf("텍스트를 조작하는 기능에 치중하였기 때문에 다른 편집기에 비해서 매우 간단한 키 입력으로 텍스트를 조작하는 것이 가능해졌습니다.\n");
    next_line();
    printf("그 당시 편집기의 주 용도는 C 프로그래밍이었습니다.\n");
    next_line();
    printf("프로그래밍의 특성 상 한번에 모든 것을 타이핑 하기 보다는 조금씩 수정하고 추가하는 일이 많았기에\n");
    printf("편집할 줄과 편집 방식을 지정하고, 방식에 따라 편집하고, 다시 돌아와서 다음에 편집할 줄과 편집 방식을 지정하는 라인 편집기 개념이 중요했습니다.\n");
    next_line();
    printf("\"vi\"는 라인 편집기 \"ed\"에서 파생되었기에 라인 편집기 개념에 기반하고 있어서 GUI에 익숙한 사용자들에게는 접하기 힘들고 친숙하지 못합니다.\n");
    next_line();
    printf("\"vi\"는 [명령 모드], [입력 모드], [마지막 행 모드]로 나눠져 있습니다.\n");
    printf("그렇기에 아무것도 모르는 초보자가 다루기에는 복잡하고 사용하기 힘들 것입니다.\n");
    next_line();
    printf("\"vi\"는 직접 많이 사용해보는 것밖에 없습니다.\n");
    next_line();
    printf("많이 사용하면서 3가지 모드를 익히고 자유자재로 다루게 된다면 vi만 쓰게 될 수도 있습니다.\n");
    next_line();
    printf("\"vi\"는 실습은 진행하지 않고 [명령 모드], [입력 모드], [마지막 행 모드]의 개념과 단축키들을 배우겠습니다.\n");
    next_line();
    printf("[명령 모드]\n");
    ...
    next_line();
    printf("[명령 모드]에서는 여러 키들로 명령을 내리거나 다른 모드로 진입을 수행하므로 텍스트가 입력되지 않습니다.\n");
    next_line();
    printf("명령 모드는 커서의 이동, 내용 수정, 삭제, 복사, 붙이기, 저장, 탐색 등을 합니다.\n");
    next_line();
    printf("어떠한 모드에 진입해 있다가 [명령 모드]로 진입하려면 \"ESC\"키를 누르시거나, Ctrl + [ 키를 누르면 됩니다.\n");
    next_line();
    printf("우선, 파일을 만들어서 내용을 입력하고 나면 파일을 저장해야 하는데 이때 쓰는 명령 키들을 익히고 가겠습니다.\n");
    next_line();
    ...
    printf("'n%%'   - 현재 파일의 내용에서 입력된 n퍼센트에 해당하는 행으로 이동합니다.\n");
    ...
    next_line();
    printf("다음은 [명령 모드]에서 수행했던 명령을 되돌리기하는 키와 검색한 결과를 볼 때 사용하는 키입니다.\n");
    next_line();
    ...
    next_line();
    printf("'i'(inset)      key  - 현재 마우스 커서의 위치부터 입력합니다.\n");
    ...
    next_line();
    printf("\"vim\"에 확장 기능을 설치하여 사용한다면 사용자의 활용도에 따라 아주 강력한 편집기가 될 수 있습니다.\n");
    printf("'https://www.vim.org/scripts/index.php'에서 필요한 것만 하나씩 받아서 적용할 수도 있으나\n");
    next_line();
    ...
    next_line();
    printf("'vimtutor'이라는 프로그램을 이용하시면 \"vi\" 혹은 \"vim\"의 사용법을 익히고 실습하실 수 있습니다.\n");
    printf("콘솔이나 터미널에 'vimtutor'을 입력하시거나, 패키지가 없다면 설치하여 튜토리얼을 진행해보시길 바랍니다.\n");
    next_line();
    printf("이후에 콘솔 혹은 터미널에 \"man\"명령어나 \"[명령어] --help\"와 같이 '--help' 옵션을 통해\n");
    printf("더 많은 정보와 명령키들을 확인하시길 바랍니다.\n");
    next_line();
