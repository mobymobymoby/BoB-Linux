...
printf "\n새로운 계정 생성 시 'root'나 'rookie'를 제외한 계정 이름으로 설정해주세요.\n\n"

while :
do
	printf "새로 생성할 계정 이름을 입력하세요 : "
	read name
	if [ "${name}" = "root" -o "${name}" = "rookie" -o "${name}" = "" ] ; then
		printf "잘못된 값을 입력하셨습니다. 다시 입력해주세요.\n\n"
	else
		break
	fi
...
fbterm
name=$(whoami)
sudo sed -i '/init_account2/d' /etc/bash.bashrc
...
printf "원활한 사용을 위해 그 후에 한번 더 재부팅하는 것을 권장드립니다.\n"
printf "(재부팅하지 않으면 exit 입력 시 Trainer가 한번 더 실행됨)\n\n"
...
printf "*When you first run this program, recommend 'reboot' Linux*\n(otherwise, Training run twice each 'exit')\n"
read trash
fbterm
exit
```
