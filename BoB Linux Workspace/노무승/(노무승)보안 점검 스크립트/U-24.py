...
# 2020/11/23 : 노무승
#   1. 실행중인 프로세스 확인 및 설정 파일 권한, 소유권, 내용을 확인하여 점검함.
#       (매뉴얼의 취약 판단 기준에서 파일 권한, 소유권 부분을 추가하고
#       삭제 방법을 리눅스 버전 별로 안내해주게끔 수정함.)
...
def U24() :
    ...
    if (temp.find("[nfsd]") != -1) or (temp.find("[statd]") != -1) or (temp.find("[lockd]") != -1) :
        ...
        if (os.path.isfile(path)):
            ...
            while temp:
                ...
                if (len(temp) != 0):
                    if (temp[0] != "#") :
                        f_output = f_output + C_YELLOW + "\t[경고] " + path + " : NFS 서버 접근 설정 파일에 내용이 있습니다.\n" + C_END
                        flag = True
                        break
                temp = handle.readline()
            handle.close()
    ...
    if (flag) :
        ...
        f_output = f_output + "\t'/etc/exports' 파일 내부 설정에서 취약한 설정은 없는지 다시 한번 확인해주세요.\n"
        f_output = f_output + "\t또한 다른 사용자가 해당 파일을 쓸 수 있는지 소유권이 root 계정이 아닌지 확인해야 합니다.\n"
